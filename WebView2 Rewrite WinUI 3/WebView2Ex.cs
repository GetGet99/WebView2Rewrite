...
public class WebView2 : UserControl
{
    ...
    Window Window;
    HWND WindowHandle;
    public WebView2(Window Window)
    {
        this.Window = Window;
        WindowHandle = new(WindowNative.GetWindowHandle(Window));
        Loaded += OnLoaded;
        Unloaded += OnUnloaded;
    ...
    void CloseInternal(bool inShutdownPath)
    {
        ...
        Microsoft.UI.Xaml.Media.CompositionTarget.Rendered -= HandleRendered;
        m_renderedRegistered = true;

        if (m_tempHostHwnd != default)
        {
    ...
    void HandlePointerWheelChanged(object sender, PointerRoutedEventArgs args)
    {
        ...
        uint message = deviceType == PointerDeviceType.Mouse ? 
            (args.GetCurrentPoint(this).Properties.IsHorizontalMouseWheel ? PInvoke.WM_MOUSEWHEEL : PInvoke.WM_MOUSEHWHEEL) : PInvoke.WM_POINTERWHEEL;
        OnXamlPointerMessage(message, args);
    }
    InputCursor? m_oldCursor;
    ...
    HWND GetHostHwnd()
    {
        ...
        {
            m_xamlHostHwnd = WindowHandle;
        }

        return m_xamlHostHwnd;
    }
    public HWND GetActiveInputWindowHwnd()
    {
    ...
    void FillPointerTouchInfo(PointerPoint inputPt, CoreWebView2PointerInfo outputPt)
    {
        ...
        //TOUCH CONTACT RAW
        double widthRaw = inputProperties.ContactRect.Width * m_rasterizationScale;
        double heightRaw = inputProperties.ContactRect.Height * m_rasterizationScale;
        double leftValRaw = inputProperties.ContactRect.X * m_rasterizationScale;
        double topValRaw = inputProperties.ContactRect.Y * m_rasterizationScale;
    ...
    unsafe void FillPointerInfo(PointerPoint inputPt, CoreWebView2PointerInfo outputPt, PointerRoutedEventArgs args)
    {
        ...
        //DEVICE TYPE
        PointerDeviceType deviceType = inputPt.PointerDeviceType;
        ...
        Point outputPt_pointerRawPixelLocation = new(m_rasterizationScale * inputPt.Position.X, m_rasterizationScale * inputPt.Position.Y);
    ...
    void UpdateCursor()
    {
        ...
        {
            ProtectedCursor = InputCursor.CreateFromCoreCursor(m_coreWebViewCompositionController.Cursor);
        }
    }
    static class WebView2Utility
    {
        ...
        PointerRoutedEventArgs args, PointerPoint pointerPoint)
        {
            ...
            else if (message == PInvoke.WM_XBUTTONDOWN || message == PInvoke.WM_XBUTTONUP)
            {
                ...
                Microsoft.UI.Input.PointerUpdateKind pointerUpdateKind = properties.PointerUpdateKind;
    ...
    void HandleAcceleratorKeyActivated(UIElement element, ProcessKeyboardAcceleratorEventArgs args)
    {
    ...
    void RegisterXamlEventHandlers()
    {
        ...
        ProcessKeyboardAccelerators += HandleAcceleratorKeyActivated;
    ...
    void TryCompleteInitialization()
    {
        ...
        if (!m_highContrastChangedRegistered)
        {
    ...
    void XamlRootChangedHelper(bool forceUpdate)
    {
        var (scale, hostVisibility) = new Func<(double, bool)>(delegate
        {
            ...
            double rawPixelsPerViewPixel = WinWrapper.Window.FromWindowHandle(WindowHandle).CurrentDisplay.ScaleFactor;
    ...
    {
        public static void ScheduleActionAfterWait(Microsoft.UI.Dispatching.DispatcherQueue Dispatcher,
    Action action,
    ...
    void UpdateCoreWebViewVisibility()
    {
        ...
        {
            SharedHelpers.ScheduleActionAfterWait(DispatcherQueue, updateCoreWebViewVisibilityAction, 200);
        }
        else
        {
            if (m_coreWebViewController != null)
    ...
    bool AreAllAncestorsVisible()
    {
        ...
        {
            if (parentAsDO is not UIElement parentAsUIE) break;
